	...
	namespace VmbAPI {
		...
		*/
					...
					if (VmbErrorSuccess == res)
					{
						...
						if (VmbErrorSuccess == res)
						{
							...
							if (VmbErrorSuccess == res)
							{
								...
								if (VmbErrorSuccess == res) std::cout << "SUCCESS starting continuous Image Acquisition " << endl;
								else  std::cout << ErrorCodeToMessage(res) << endl;
							}
						}
					}
				}
		...
		VmbFrameStatusType ApiController::GetFrame(cv::Mat& m)
		{
			...
			{
				std::cout << "Problems reading status, skipping frame..." << endl;
				std::cout << ErrorCodeToMessage(VmbErrorSuccess) << endl;
			...
			if (VmbFrameStatusComplete == status)
			{
				...
				if (err == VmbErrorSuccess)
				{
					cout << "Image successfully acquired..." << endl;
					try
					{
				...
				{
					std::cout << "Could not get the image, skipping it." << endl;
					status = VmbFrameStatusInvalid;
				}
			}
			else std::cout << "Corrupted Frame Skipping..." << endl;
		...
		VmbFrameStatusType ApiController::GetFrame(cv::Mat& m,CameraPtr pCamera, IFrameObserverPtr pFrameObserver)
		{
			...
			{
				std::cout << "Waiting for Frame to arrive" << endl;
				while (!FrameAvailable(pFrameObserver)) continue;
				Sleep(100); // frame needs to get copied before we can read it. Currently there is no way to check when this happened.
			}
